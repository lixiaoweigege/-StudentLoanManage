...
function openContract(){
	...
	var row=selectedRows[0];
	//实现页面的跳转
	   window.location.href='openContract.action?id='+row.id+"&pidcard="+row.pidcard+"&ayear="+row.ayear+"&aterm="+row.aterm;
}
...
pagination="true" rownumbers="true" fit="true" fitColumns="true" nowrap="true"   url="findAllLoanApply.action"
 > 
	...
	<div id="toolbar">
		<div>		
			<a href="javascript:openEditDialog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">审核意见</a>
			<a href="javascript:openContract()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">填写贷款合同</a>		   
		</div>
		<div>
		<table><tr><td>身份证号&nbsp; <input type="text" name="s_pidcard" id="s_pidcard" width="20px">&nbsp;
		    ...
		    <select  name="s_astate" id="s_astate" style="width: 120px;" >
		    <option value=""></option>
		    <option value="待审核">待审核</option>
		    <option value="驳回申请">驳回申请</option>
		    <option value="通过审核">通过审核</option>
		     <option value="合同已签">合同已签</option>
